...
/*
* 扩展加了多波形图不同颜色显示参数
* waveColorArr 和 progressColorArr
*
* */
  ...
  _createClass(MultiCanvas, [{
    ...
    value: function drawWave(peaks, channelIndex, start, end) {
      ...
      return this.prepareDraw(peaks, channelIndex, start, end, function (_ref2) {
        ...
        if (start !== undefined) {
          _this5.drawLine(peaks, absmax, halfH, offsetY, start, end,index);//增加传入index参数（mg)
        } // Always draw a median line


        _this5.fillRect(0, halfH + offsetY - _this5.halfPixel, _this5.width, _this5.halfPixel,index);//增加传入index参数（mg)
      });
    }
    /**
    ...
    key: "drawLine",
    value: function drawLine(peaks, absmax, halfH, offsetY, start, end,index) {
      var _this6 = this;

      this.canvases.forEach(function (entry) {
        _this6.setFillStyles(entry,index);//增加传入index参数（mg)
    ...
    key: "fillRect",
    value: function fillRect(x, y, width, height,index) {
      ...
      for (i = startCanvas; i < endCanvas; i++) {
        ...
        if (intersection.x1 < intersection.x2) {
          this.setFillStyles(entry,index);
    ...
    key: "prepareDraw",
    value: function prepareDraw(peaks, channelIndex, start, end, fn,index) {//增加了index参数(mg)
      var _this7 = this;
        ...
        if (peaks[0] instanceof Array) {
          ...
          if (_this7.params.splitChannels) {
            ...
            return channels.forEach(function (channelPeaks, i) {
              return _this7.prepareDraw(channelPeaks, i, start, end, fn,i);
            });
          }
    ...
    key: "setFillStyles",
    value: function setFillStyles(entry,index) {
      var wca=this.params.waveColorArr;
      if(index>=0&&wca&&wca.length>index&&wca[index]){
            entry.waveCtx.fillStyle = wca[index];
      }else{
      ...
      if (this.hasProgressCanvas) {
        ...
        var pca = this.params.progressColorArr;
          if(index>=0&&pca&&pca.length>index&&pca[index]){
              entry.progressCtx.fillStyle = pca[index];
          }else{
              entry.progressCtx.fillStyle = this.params.progressColor;
          }
      }
...
function (_util$Observer) {
  ...
  _createClass(WebAudio, [{
     ...
     */
      ...
      {
          key: "createChannelNodes",
          value: function createChannelNodes() {
              var channels = this.buffer.numberOfChannels;
      ...
      },
      {
          ...
          value: function setChannel(channel) {
              ...
              if (channel == undefined || channel === -1) {
                  //console.log("===stereo play===")
                  for (var c = 0; c < channels; c++) {
                      this.splitter.connect(this.merger, c, c);
                  }
              }
              else if (channel == 0 || channel == 1) {
                  //console.log("===left/right channel===")
